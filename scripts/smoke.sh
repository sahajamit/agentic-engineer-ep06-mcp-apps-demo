...
(
  ...
  sleep 1
  # Second search MUST land on a different slot URI so Claude Desktop's
  # per-URI cache is busted.
  printf '{"jsonrpc":"2.0","id":5,"method":"tools/call","params":{"name":"search_products","arguments":{"query":"running shoes","minPrice":60}}}\n'
  sleep 2
...
# Per-call _meta override (cache-bust fix): each search must advertise its
# own slot URI so Claude Desktop refetches instead of serving the stale
# carousel.
grep -q '"resourceUri":"ui://agentic-engineer-storefront/widgets/search/v0"' "$OUT" || { echo "✗ first search slot URI (search/v0) missing"; fail=1; }
grep -q '"resourceUri":"ui://agentic-engineer-storefront/widgets/search/v1"' "$OUT" || { echo "✗ second search slot URI (search/v1) missing"; fail=1; }
